...
public class SalaryService {
    ...
    public SalaryService(StudentRepository studentRepository, LessonService lessonService) {
    ...
    private Double calculate(LocalDateTime start, LocalDateTime end, List<Student> students) {
        List<Student> studentsByGroup = students.stream().filter(student -> student.getLessons().stream()
                .anyMatch(lesson -> lesson.getType() == LessonType.GROUP))
            .collect(Collectors.toList());

        List<Student> studentsNoGroup = students.stream().filter(student -> student.getLessons().stream()
                .anyMatch(lesson -> lesson.getType() == LessonType.SINGLE))
            .collect(Collectors.toList());
