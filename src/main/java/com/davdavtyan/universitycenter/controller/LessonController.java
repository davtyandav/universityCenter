...
public class LessonController {
    ...
    // ЭНДПОИНТ ДЛЯ ЗАВЕРШЕНИЯ УРОКА (Отметка посещаемости + добавление задач)
    @PutMapping("/{id}/complete")
    public ResponseEntity<LessonResponse> completeLesson(@PathVariable Long id,
                                                         @RequestBody LessonCompleteRequest completeRequest) {
        Lesson completedLesson = lessonService.completeLessonWithDetails(id, completeRequest);
        return ResponseEntity.ok(LessonConverter.toDto(completedLesson));
    }
