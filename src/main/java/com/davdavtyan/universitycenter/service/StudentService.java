...
public class StudentService {
    ...
    public Student createStudent(Student student, Long mentorId) {
        ...
        if (mentor != null) {
            List<Student> attach = attach(mentor, Collections.singletonList(save.getId()));
        }
    ...
    public List<Student> attach(Long mentorId, List<Long> studentsIds) {
        ...
        return attach(mentor, studentsIds);
    }

    private List<Student> attach(Mentor mentor, List<Long> students) {
        List<Student> studentsWithMentor = studentRepository.findAllById(students)
            .stream()
            .map(student -> {
                student.setMentor(mentor);
                return studentRepository.save(student);
